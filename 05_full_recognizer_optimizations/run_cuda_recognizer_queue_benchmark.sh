...
if [[ -z "${HF_HOME:-}" && -d /workspace/.hf_home ]]; then
  export HF_HOME=/workspace/.hf_home
fi
export HF_HUB_DISABLE_XET="${HF_HUB_DISABLE_XET:-1}"
export HF_XET_DISABLE="${HF_XET_DISABLE:-1}"
...
OUTPUT_PATH="${OUTPUT_PATH:-${OUTPUT_DIR}/recognizer_queue_cuda_${RUN_ID}_${DECODE_SCHEDULE}_n${NUM_ITEMS}_b${ACTIVE_BATCH_SIZE}_vp${VISION_PREFILL_BATCH_SIZE}_cache${CACHE_LENGTH}_${DECODE_BACKEND}.json}"
