...
CROP_IDS="${CROP_IDS:-}"
OUTPUT_DIR="${OUTPUT_DIR:-${SCRIPT_DIR}/outputs/attention_softmax_compare_npu}"
...
echo "NPU_ATTENTION_SOFTMAX_COMPARE_RUN fp32 text+vision -> ${BASELINE_PATH}"
run_one fp32 fp32 "${BASELINE_PATH}"

echo "NPU_ATTENTION_SOFTMAX_COMPARE_RUN model text+vision -> ${CANDIDATE_PATH}"
...
print("NPU_ATTENTION_SOFTMAX_COMPARE_SUMMARY", json.dumps(summary, sort_keys=True))
if mismatches:
    raise SystemExit(f"fp32 vs model/fp16 attention generated different outputs for {len(mismatches)} item(s)")
PY

echo "NPU_ATTENTION_SOFTMAX_COMPARE_PASS ${BASELINE_PATH} ${CANDIDATE_PATH}"
