...
export PROMPTFA_LAYOUT="${PROMPTFA_LAYOUT:-bnsd}"
export PROMPTFA_PAD_HEAD_DIM_TO="${PROMPTFA_PAD_HEAD_DIM_TO:-0}"
export INPUT_BOUNDARY="${INPUT_BOUNDARY:-bnsd}"
...
SAFE_NAME="attn_${ATTENTION}_layout_${PROMPTFA_LAYOUT}_padD_${PROMPTFA_PAD_HEAD_DIM_TO}_boundary_${INPUT_BOUNDARY}_backend_${COMPILE_BACKEND}_mask_${MASK_KIND}_rank_${MASK_RANK}_nopad_${NO_PADDING}_eagerly_${TORCHAIR_RUN_EAGERLY}"
...
echo "EXP07_ATTENTION_ONLY PROMPTFA_LAYOUT=${PROMPTFA_LAYOUT}"
echo "EXP07_ATTENTION_ONLY PROMPTFA_PAD_HEAD_DIM_TO=${PROMPTFA_PAD_HEAD_DIM_TO}"
echo "EXP07_ATTENTION_ONLY INPUT_BOUNDARY=${INPUT_BOUNDARY}"
echo "EXP07_ATTENTION_ONLY COMPILE_BACKEND=${COMPILE_BACKEND}"
echo "EXP07_ATTENTION_ONLY TORCHAIR_MODE=${TORCHAIR_MODE}"
...
"${PYTHON_BIN}" "${SCRIPT_DIR}/repro_attention_only_compile.py" \
  ...
  --npu-jit-compile off \
  --attention "${ATTENTION}" \
  --promptfa-layout "${PROMPTFA_LAYOUT}" \
