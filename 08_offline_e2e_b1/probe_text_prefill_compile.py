...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--torchair-cache-dir",
        type=Path,
        default=Path(".runtime_cache/08_offline_e2e_b1_text_torchair_probe"),
    )
...
def main() -> None:
    ...
    configure_npu_jit_compile("off", device)
    ...
    runtime = BucketedTextPrefillRuntime(
        model,
        backend="torchair",
        buckets=(args.bucket,),
    ...
    eager_hidden = model.model.forward_prefill_static(
        inputs_embeds,
        attention_mask,
        position_ids,
        eager_cache,
    ...
    compiled_hidden = runtime.run_prepared(prepared, compiled_cache)
    synchronize(device)
    ...
    for _ in range(args.repeats):
        ...
        model.model.forward_prefill_static(
            inputs_embeds,
            attention_mask,
            position_ids,
            cache,
        ...
        synchronize(device)
        started = time.perf_counter()
        runtime.run_prepared(prepared, cache)
        synchronize(device)
    ...
    result = {
        ...
        "compile": runtime.compile_metadata,
        "correctness": {
