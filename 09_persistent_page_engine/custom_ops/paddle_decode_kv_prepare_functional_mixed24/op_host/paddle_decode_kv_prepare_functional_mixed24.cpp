#include "../op_kernel/paddle_decode_kv_prepare_functional_mixed24_tiling.h"
#include "register/op_def_registry.h"

namespace optiling {
static bool HasShape(
...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
    ...
    auto* tiling =
        context->GetTilingData<PaddleDecodeKvPrepareFunctionalMixed24TilingData>();
    tiling->cacheElements = 2 * 1024 * 128;
    tiling->copyElementsPerCore = 10928;
...
namespace ops {
class PaddleDecodeKvPrepareFunctionalMixed24 : public OpDef {
public:
    explicit PaddleDecodeKvPrepareFunctionalMixed24(const char* name)
        : OpDef(name)
    {
        ...
        this->Output("attention_mask").ParamType(REQUIRED).DataType({ge::DT_BOOL})
            .Format({ge::FORMAT_ND}).UnknownShapeFormat({ge::FORMAT_ND});
        this->Output("key_cache_out").ParamType(REQUIRED).DataType({ge::DT_FLOAT16})
            .Format({ge::FORMAT_ND}).UnknownShapeFormat({ge::FORMAT_ND});
        this->Output("value_cache_out").ParamType(REQUIRED).DataType({ge::DT_FLOAT16})
            .Format({ge::FORMAT_ND}).UnknownShapeFormat({ge::FORMAT_ND});
...
OP_ADD(PaddleDecodeKvPrepareFunctionalMixed24);
}
