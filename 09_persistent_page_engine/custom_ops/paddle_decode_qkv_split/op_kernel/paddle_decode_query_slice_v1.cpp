...
namespace {
constexpr uint32_t kElements = 2048;

class PaddleDecodeQuerySliceKernel {
public:
    __aicore__ inline void Init(GM_ADDR qkv, GM_ADDR output, TPipe* pipe)
    {
        inputGm.SetGlobalBuffer(reinterpret_cast<__gm__ half*>(qkv), kElements);
...
extern "C" __global__ __aicore__ void paddle_decode_query_slice_v1(
    GM_ADDR qkv,
    GM_ADDR output,
    GM_ADDR workspace,
    GM_ADDR tiling)
