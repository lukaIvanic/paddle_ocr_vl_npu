"""Mixed-task attention-only B1/KV1024 GQA operator for SuperKernel."""
...
PYTORCH_OP_NAME = "paddleocr_vl::decode_gqa_attention_mixed24"
GE_OP_NAME = "PaddleDecodeGqaAttentionMixed24"
INPUT_LAYOUT = "BNSD"


@torch.library.custom_op(PYTORCH_OP_NAME, mutates_args=())
def _decode_gqa_attention_mixed24(
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
...
@_decode_gqa_attention_mixed24.register_fake
def _decode_gqa_attention_mixed24_fake(
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
    atten_mask: torch.Tensor,
...
def register_decode_gqa_attention_mixed24_converter() -> None:
    """Lower to the independent 24-worker mixed-task GE identity."""
    ...
    @register_converter(
        torch.ops.paddleocr_vl.decode_gqa_attention_mixed24.default
    )
    def _convert_decode_gqa_attention_mixed24(
        query: Any,
        key: Any,
        value: Any,
        atten_mask: Any,
...
def decode_gqa_attention_mixed24(
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
    atten_mask: torch.Tensor,
    *,
    ...
    """Run 16 attention workers inside a 24-AIV/24-AIC launch contract."""
    if query.shape != (1, 16, 1, 128):
        raise ValueError("mixed24 GQA attention requires query[1,16,1,128]")
    if key.shape != (1, 2, 1024, 128) or value.shape != key.shape:
        raise ValueError("mixed24 GQA attention requires K/V[1,2,1024,128]")
    if atten_mask.shape != (1, 1, 1, 1024) or atten_mask.dtype != torch.bool:
        raise ValueError("mixed24 GQA attention requires bool mask[1,1,1,1024]")
    if any(tensor.dtype != torch.float16 for tensor in (query, key, value)):
        raise ValueError("mixed24 GQA attention requires FP16 Q/K/V")
    if num_heads != 16 or num_key_value_heads != 2:
        raise ValueError("mixed24 GQA attention fixes 16 query and 2 KV heads")
    if inner_precise != 1 or vector_core_count != 16:
        raise ValueError(
            "mixed24 GQA attention fixes inner_precise=1 and 16 workers"
        )
    return _decode_gqa_attention_mixed24(
        query.contiguous(),
        key.contiguous(),
        value.contiguous(),
