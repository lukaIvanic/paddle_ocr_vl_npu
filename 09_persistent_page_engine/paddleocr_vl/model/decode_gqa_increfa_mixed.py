...
def _decode_gqa_incre_flash_attention_mixed24(
    query: torch.Tensor,
    key_cache: torch.Tensor,
    value_cache: torch.Tensor,
    attention_mask: torch.Tensor,
    ...
    """Use the same stock eager reference under the independent 24-AIV identity."""
    import torch_npu
...
@_decode_gqa_incre_flash_attention_mixed24.register_fake
def _decode_gqa_incre_flash_attention_mixed24_fake(
    query: torch.Tensor,
    key_cache: torch.Tensor,
    value_cache: torch.Tensor,
    attention_mask: torch.Tensor,
...
def register_decode_gqa_increfa_mixed_converter() -> None:
    ...
    @register_converter(
        torch.ops.paddleocr_vl.decode_gqa_incre_flash_attention_mixed24.default
    )
    def _convert_decode_gqa_incre_flash_attention_mixed24(
        query: Any,
        key_cache: Any,
        value_cache: Any,
        attention_mask: Any,
...
def register_decode_gqa_increfa_mixed24_converter() -> None:
    """Register the independent 24-AIV hardware-barrier GE identity."""
...
def decode_gqa_incre_flash_attention_mixed24(
    query: torch.Tensor,
    key_cache: torch.Tensor,
    value_cache: torch.Tensor,
    attention_mask: torch.Tensor,
    ...
    *,
    ...
    """Execute 16-worker GQA after a geometry-matched 24-AIV barrier."""
    if query.shape != (1, 16, 1, 128):
        raise ValueError("mixed24 fused GQA requires query[1,16,1,128]")
    if key_cache.shape != (1, 2, 1024, 128):
        raise ValueError("mixed24 fused GQA requires key cache[1,2,1024,128]")
    if value_cache.shape != key_cache.shape:
        raise ValueError("mixed24 fused GQA requires matching K/V caches")
    if key_state.shape != (1, 2, 1, 128) or value_state.shape != key_state.shape:
        raise ValueError("mixed24 fused GQA requires state[1,2,1,128]")
    if attention_mask.shape != (1, 1, 1, 1024):
        raise ValueError("mixed24 fused GQA requires mask scratch[1,1,1,1024]")
    if attention_mask.dtype != torch.bool:
        raise ValueError("mixed24 fused GQA requires a bool mask scratch")
    if cache_position.shape != (1,) or cache_position.dtype != torch.int64:
        raise ValueError("mixed24 fused GQA requires int64 cache position[1]")
    if any(
        tensor.dtype != torch.float16
        for tensor in (query, key_cache, value_cache, key_state, value_state)
    ):
        raise ValueError("mixed24 fused GQA requires FP16 Q/K/V/state tensors")
    if num_heads != 16 or num_key_value_heads != 2:
        raise ValueError("mixed24 fused GQA fixes 16 query and 2 KV heads")
    if inner_precise != 1 or vector_core_count != 16:
        raise ValueError("mixed24 fused GQA fixes inner_precise=1 and 16 attention workers")
    return _decode_gqa_incre_flash_attention_mixed24(
        query.contiguous(),
        key_cache,
        value_cache,
        attention_mask,
