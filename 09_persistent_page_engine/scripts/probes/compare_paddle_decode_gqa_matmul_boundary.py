#!/usr/bin/env python3
"""Validate the fused B1 decode-GQA to output-projection boundary."""
...
class DecodeGqaMatMulBoundary(torch.nn.Module):
    ...
    def _forward_impl(
        ...
        attention = decode_gqa_incre_flash_attention_aiv(
            query,
            key_cache,
            value_cache,
            attention_mask,
        ...
        flattened = attention.transpose(1, 2).contiguous().reshape(1, 2048)
        return decode_linear_matmul_v3(flattened, output_weight)
    ...
    def forward(
        ...
        with self.scope(
            "paddle_decode_gqa_matmul_boundary",
...
def main() -> int:
    ...
    output_weight = torch.randn(
        (1024, 2048), generator=generator, dtype=torch.float16
    ).to("npu:0")
    output_weight = torch_npu.npu_format_cast(output_weight, FRACTAL_NZ)

    torchair, CompilerConfig = import_torchair()
    args.cache_dir.mkdir(parents=True, exist_ok=True)
    step = torchair.inference.cache_compile(
        DecodeGqaMatMulBoundary(args.super_kernel_options).forward,
    ...
    for position, key_state, value_state in zip(
        (128, 129), key_states, value_states, strict=True
    ):
        ...
        reference_attention = torch_npu.npu_incre_flash_attention(
            query,
            ref_key_cache,
            ref_value_cache,
        ...
        reference = F.linear(
            reference_attention.transpose(1, 2).contiguous().reshape(1, 2048),
            output_weight,
        )
        torch.npu.synchronize()
    ...
    result = {
        "kind": "paddle_decode_gqa_matmul_boundary_probe",
        "contract": {
            "query_shape": list(query.shape),
            "cache_shape": list(key_cache.shape),
            "output_weight_shape": list(output_weight.shape),
            "output_weight_npu_format": int(
                torch_npu.get_npu_format(output_weight)
            ),
            "positions": [128, 129],
            "vector_core_count": 16,
    ...
    if not all_close:
        raise RuntimeError("fused decode GQA to MatMulV3 boundary failed parity")
    return 0
