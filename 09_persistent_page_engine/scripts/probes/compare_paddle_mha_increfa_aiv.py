#!/usr/bin/env python3
"""Compare stock IncreFA with the separately named Paddle MHA AIV op."""
...
from paddleocr_vl.model.mha_increfa_aiv import (
    GE_OP_NAME,
    PYTORCH_OP_NAME,
    mha_incre_flash_attention_aiv,
    register_mha_increfa_aiv_converter,
)
...
class StockIncreFa(torch.nn.Module):
    def forward(
        self,
        query: torch.Tensor,
        key: torch.Tensor,
...
class PaddleMhaAiv(torch.nn.Module):
    def forward(
        self,
        query: torch.Tensor,
        key: torch.Tensor,
        ...
        return mha_incre_flash_attention_aiv(
            query,
            key,
            value,
            mask,
...
def time_step(
    ...
    *,
    ...
    for _ in range(blocks):
        ...
        host_us.append(host_elapsed * 1e6 / repeats_per_block)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result: dict[str, Any] = {
        "schema_version": 1,
        "kind": "separate_paddle_mha_increfa_aiv_comparison",
        "operator_identity": {
