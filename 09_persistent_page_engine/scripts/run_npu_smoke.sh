...
OUTPUT_DIR="${OUTPUT_DIR:-$REPO_ROOT/tmp/09_persistent_page_engine/smoke}"
...
if [[ -n "${VISION_PADDING:-}" ]]; then
  args+=(--vision-padding "$VISION_PADDING")
fi
if [[ -n "${TEXT_BACKEND:-}" ]]; then
  args+=(--text-backend "$TEXT_BACKEND")
fi
if [[ -n "${TEXT_PADDING:-}" ]]; then
  args+=(--text-padding "$TEXT_PADDING")
fi
...
if [[ -n "${VISION_BUCKETS:-}" ]]; then
  args+=(--vision-buckets "$VISION_BUCKETS")
fi
if [[ -n "${TEXT_BUCKETS:-}" ]]; then
  args+=(--text-buckets "$TEXT_BUCKETS")
fi
