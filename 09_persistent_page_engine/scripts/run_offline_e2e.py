...
from pipeline.layout import PPDocLayoutV3Runtime
from pipeline.page_pipeline import OfflinePagePipeline, aggregate_pages
from pipeline.types import RunResult


DEFAULT_OUTPUT_ROOT = REPO_ROOT / "tmp" / "09_persistent_page_engine"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Padding policy independent of execution backend; auto buckets only for TorchAir.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Strictly increasing comma-separated B=1 text-prefill padding shapes.",
    )
    parser.add_argument(
        "--text-padding",
        default="auto",
...
def main() -> None:
    ...
    result = RunResult(
        experiment="09_persistent_page_engine",
        configuration=configuration,
        setup_timing_s={
            "npu_runtime_init": float(npu_runtime_init_s),
