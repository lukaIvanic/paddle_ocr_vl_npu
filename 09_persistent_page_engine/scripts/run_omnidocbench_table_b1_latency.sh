...
SERVER_PYTHON="${SERVER_PYTHON:-${PYTHON:-/workspace/venvs/vllm_paddle_ocr_pipeline_py312/bin/python}}"
CLIENT_PYTHON="${CLIENT_PYTHON:-/workspace/venvs/omnidocbench_py310/bin/python}"
...
"$SERVER_PYTHON" - "$PORT" "$SERVER_PID" "$SERVER_LOG" <<'PY'
import json
import os
...
"$CLIENT_PYTHON" 09_persistent_page_engine/scripts/run_omnidocbench_table_api.py \
  "${CLIENT_ARGS[@]}" \
  2>&1 | tee "$CLIENT_LOG"
