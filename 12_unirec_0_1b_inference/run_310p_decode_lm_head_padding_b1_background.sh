...
LAB="$SCRIPT_DIR/decode_lm_head_padding_b1_lab.py"
...
resolve_inputs() {
  ...
  CACHE_PARENT="${CACHE_PARENT:-$REPO/.runtime_cache/12_unirec_0_1b_inference/decode_lm_headpad_b1_310p}"
...
run_lane() {
  local lane="$1" rows="$2" lane_root child status started now profile_dir
  ...
  env PYTHONUNBUFFERED=1 TORCH_LOGS=recompiles \
    ...
    "$PYTHON_BIN" "$LAB" --model "$MODEL" --device npu:0 \
      ...
      --lm-head-rows "$rows" --source-length 56 --warmup-steps 20 \
      --measure-steps 100 --timing-steps 100 --validation-steps 100 \
  ...
  while kill -0 "$child" 2>/dev/null; do
    sleep 10
    now="$(date +%s)"
    printf 'UNIREC_310P_DECODE_LM_HEAD_PADDING_B1_HEARTBEAT lane=%s elapsed_s=%s compiler_processes=%s om_count=%s last_event=%q\n' \
      "$lane" "$((now - started))" "$(compiler_count)" \
      "$(find "$CACHE_PARENT" -type f -name '*.om' 2>/dev/null | wc -l)" \
      "$(grep 'UNIREC_DECODE_LM_HEAD_PADDING_B1_PROGRESS' "$lane_root/run.log" | tail -n 1 || true)"
  done
  set +e
  wait "$child"
  status="$?"
  ...
  printf 'UNIREC_310P_DECODE_LM_HEAD_PADDING_B1_LANE_END lane=%s status=%s wall_s=%s\n' \
...
base = json.load(open(root / "vocab56371/result.json"))
aligned = json.load(open(root / "vocab57344/result.json"))
a = np.load(root / "vocab56371/result.validation_logits.npy").astype(np.float64)
b = np.load(root / "vocab57344/result.validation_logits.npy").astype(np.float64)
assert a.shape == b.shape == (1, 1, 56371)
delta = np.abs(a - b)
cosine = float(
...
def profile(lane, vocab):
    ...
    matmuls = kernels["top_matmul_shape_signatures"]
    lm_head = [row for row in matmuls if str(vocab) in row["name"]]
    return {
...
base_tokens = base["validation_tokens"][0]
aligned_tokens = aligned["validation_tokens"][0]
first_mismatch = next(
    (i for i, (x, y) in enumerate(zip(base_tokens, aligned_tokens)) if x != y),
    None,
)
report = {
    "status": "pass",
    "baseline": {
        "shape": base["shape"],
        "timing": timing(base),
        "profile": profile("vocab56371", 56371),
    },
    "aligned": {
        "shape": aligned["shape"],
        "timing": timing(aligned),
        "profile": profile("vocab57344", 57344),
    },
    "aligned_over_baseline": {
        "raw_tok_s_speedup": aligned["raw_tok_s"] / base["raw_tok_s"],
        "step_speedup": base["step_ms"] / aligned["step_ms"],
        "production_like_wall_speedup": (
            base["compiled_timing"]["production_like_d2h"]["wall_step_ms"]
            / aligned["compiled_timing"]["production_like_d2h"]["wall_step_ms"]
        ),
    },
    "parity": {
        ...
        "validation_tokens_exact": base_tokens == aligned_tokens,
        "validation_token_count": len(base_tokens),
        "first_token_mismatch": first_mismatch,
    },
    "run_root": str(root),
}
assert np.isfinite(a).all() and np.isfinite(b).all()
assert base["weights_nz"] and aligned["weights_nz"]
assert base["nz_tensor_count"] == aligned["nz_tensor_count"] == 49
assert base["logits_rows_returned"] == aligned["logits_rows_returned"] == 56371
assert report["parity"]["argmax_exact"]
(root / "comparison.json").write_text(json.dumps(report, indent=2) + "\n")
print("UNIREC_310P_DECODE_LM_HEAD_PADDING_B1: PASS")
print(
    "UNIREC_310P_DECODE_LM_HEAD_PADDING_B1_RESULT "
...
worker_main() {
  ...
  run_lane vocab56371 56371
  run_lane vocab57344 57344
...
launch_main() {
  ...
  RUN_ROOT="${RUN_ROOT:-$REPO/tmp/12_unirec_0_1b_inference/310p_decode_lm_headpad_b1_${short}_${stamp}}"
