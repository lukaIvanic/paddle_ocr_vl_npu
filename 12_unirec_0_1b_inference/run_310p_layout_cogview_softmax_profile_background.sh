...
ANALYZER="$SCRIPT_DIR/compare_layout_cogview_softmax_profile.py"
...
resolve_inputs() {
  ...
  : "${FROZENBN_RUN_ROOT:?export the completed 310P FrozenBN run root}"
  ...
  if [[ "$ASCEND_RT_VISIBLE_DEVICES" == *,* ]]; then
    printf 'LAYOUT_COGVIEW_REQUIRES_ONE_NPU=%s\n' \
  ...
  FROZENBN_RUN_ROOT="$(readlink -f "$FROZENBN_RUN_ROOT")"
  ...
  test -f "$FROZENBN_RUN_ROOT/exit_code.txt"
  test "$(tr -d '[:space:]' <"$FROZENBN_RUN_ROOT/exit_code.txt")" = 0
  test -f "$LAYOUT_LAB"
  test -f "$PROFILE_RUNNER"
  test -f "$ANALYZER"

  BASELINE_FORWARD="$FROZENBN_RUN_ROOT/forward_constant_grouped_frozenbn.json"
  BASELINE_PROFILE="$FROZENBN_RUN_ROOT/profile_frozenbn/profile_suite_summary.json"
...
run_phase() {
  ...
  local phase_log="$2"
  shift 2
  printf 'UNIREC_LAYOUT_COGVIEW_PHASE_BEGIN phase=%s command=' "$phase"
  printf '%q ' "$@"
  printf '\n'
  ...
  status="$?"
  set -e
  printf 'UNIREC_LAYOUT_COGVIEW_PHASE_END phase=%s status=%s wall_s=%s\n' \
    "$phase" "$status" "$((SECONDS - started))"
  return "$status"
}

run_forward() {
  local output="$RUN_ROOT/forward_direct_softmax.json"
  local command=(
    ...
    --cogview-attention-impl direct_softmax
...
run_profile() {
  local output="$RUN_ROOT/profile_direct_softmax"
  local command=(
    ...
    --layout-cogview-attention-impl direct_softmax
    --warmup 2
    --control-repeats 20
    --profile-steps 1
    --profile-metric pipe
...
summarize() {
  local command=(
    ...
    --candidate-forward "$RUN_ROOT/forward_direct_softmax.json"
    --candidate-profile "$RUN_ROOT/profile_direct_softmax/profile_suite_summary.json"
...
worker_main() {
...

  {
    ...
    printf 'frozenbn_run_root=%s\nlayout_cache=%s\n' \
      "$FROZENBN_RUN_ROOT" "$LAYOUT_CACHE"
...
worker_entry() {
  ...
  printf 'UNIREC_310P_LAYOUT_COGVIEW_END status=%s run_log=%s\n' \
...
launch_main() {
  ...
  RUN_ROOT="${RUN_ROOT:-$REPO/tmp/12_unirec_0_1b_inference/310p_layout_cogview_${commit_short}_${timestamp}}"
  LAYOUT_CACHE="${LAYOUT_CACHE:-$REPO/.runtime_cache/12_unirec_0_1b_inference/310p_layout_cogview_${commit_short}_${timestamp}}"
  ...
  nohup env \
    ...
    FROZENBN_RUN_ROOT="$FROZENBN_RUN_ROOT" LAYOUT_CACHE="$LAYOUT_CACHE" \
  ...
  sleep 1
  kill -0 "$pid"
  printf 'UNIREC_310P_LAYOUT_COGVIEW_STARTED pid=%s physical=%s\n' \
