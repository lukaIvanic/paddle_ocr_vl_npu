...
ANALYZER="$SCRIPT_DIR/compare_layout_frozenbn_profile.py"
...
resolve_inputs() {
  ...
  : "${CONSTANT_GROUPED_RUN_ROOT:?export the completed 310P constant-grouped run root}"
  ...
  if [[ "$ASCEND_RT_VISIBLE_DEVICES" == *,* ]]; then
    printf 'LAYOUT_FROZENBN_REQUIRES_ONE_NPU=%s\n' \
  ...
  CONSTANT_GROUPED_RUN_ROOT="$(readlink -f "$CONSTANT_GROUPED_RUN_ROOT")"
  ...
  test -f "$CONSTANT_GROUPED_RUN_ROOT/exit_code.txt"
  test "$(tr -d '[:space:]' <"$CONSTANT_GROUPED_RUN_ROOT/exit_code.txt")" = 0
  test -f "$LAYOUT_LAB"
  test -f "$PROFILE_RUNNER"
  test -f "$ANALYZER"

  BASELINE_FORWARD="$CONSTANT_GROUPED_RUN_ROOT/forward_constant_grouped.json"
  BASELINE_PROFILE="$CONSTANT_GROUPED_RUN_ROOT/profile_constant_grouped/profile_suite_summary.json"
...
run_phase() {
  ...
  local phase_log="$2"
  shift 2
  printf 'UNIREC_LAYOUT_FROZENBN_PHASE_BEGIN phase=%s command=' "$phase"
  printf '%q ' "$@"
  printf '\n'
  ...
  status="$?"
  set -e
  printf 'UNIREC_LAYOUT_FROZENBN_PHASE_END phase=%s status=%s wall_s=%s\n' \
    "$phase" "$status" "$((SECONDS - started))"
  return "$status"
}

run_forward() {
  local output="$RUN_ROOT/forward_constant_grouped_frozenbn.json"
  local command=(
...
run_profile() {
  local output="$RUN_ROOT/profile_frozenbn"
  local command=(
...
summarize() {
  local command=(
    ...
    --candidate-forward "$RUN_ROOT/forward_constant_grouped_frozenbn.json"
    --candidate-profile "$RUN_ROOT/profile_frozenbn/profile_suite_summary.json"
...
worker_main() {
...

  {
    ...
    printf 'constant_grouped_run_root=%s\nlayout_cache=%s\n' \
      "$CONSTANT_GROUPED_RUN_ROOT" "$LAYOUT_CACHE"
...
worker_entry() {
  ...
  printf 'UNIREC_310P_LAYOUT_FROZENBN_END status=%s run_log=%s\n' \
...
launch_main() {
  ...
  RUN_ROOT="${RUN_ROOT:-$REPO/tmp/12_unirec_0_1b_inference/310p_layout_frozenbn_${commit_short}_${timestamp}}"
  LAYOUT_CACHE="${LAYOUT_CACHE:-$REPO/.runtime_cache/12_unirec_0_1b_inference/310p_layout_frozenbn_${commit_short}_${timestamp}}"
  ...
  nohup env \
    ...
    CONSTANT_GROUPED_RUN_ROOT="$CONSTANT_GROUPED_RUN_ROOT" \
  ...
  sleep 1
  kill -0 "$pid"
  printf 'UNIREC_310P_LAYOUT_FROZENBN_STARTED pid=%s physical=%s\n' \
