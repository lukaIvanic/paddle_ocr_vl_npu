...
if [[ "${1:-}" != "--worker" ]]; then
  ...
  RUN_ROOT="${RUN_ROOT:-$REPO/tmp/12_unirec_0_1b_inference/310p_standalone_vision_stage1_trace_$(date +%Y%m%dT%H%M%S)}"
...
printf 'UNIREC_STANDALONE_STAGE1_TRACE_BEGIN epoch_s=%s expected_graphs=1\n' "$(date +%s)"
...
printf 'UNIREC_STANDALONE_STAGE1_TRACE_END epoch_s=%s\n' "$(date +%s)"
