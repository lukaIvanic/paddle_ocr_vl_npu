...
## Phase 31: decode-hang discriminator matrix

### 31.0 Why this phase replaces further pipeline replays

The stalling decode step is **shape-identical to every other decode step**.
`paddleocr_vl/model/text_decode.py` calls `npu_incre_flash_attention` with
`actual_seq_lengths=None` over the entire physical KV cache, and excludes the
unwritten tail with a boolean mask built from `cache_position`. The compiled
graph, its tiling, and every tensor shape are therefore the same at effective
length 1279, 1280, and 1281. Only tensor *values* differ.

That rules out a bucket or graph-shape boundary as the cause, and it means the
remaining suspects can only be separated by changing one execution property at
a time. It also exposes a confound present in every run so far: the target
crop always reaches effective length 1280 on its 259th generated token, so
"absolute position 1280" and "this crop's 259th token" have never been
separated.

Run every lane in this phase in one session. They are independent, each is
minutes at most, and a partial matrix does not discriminate.

Matched 910B2 controls at commit `4fa19b4` are committed under
`tmp/09_persistent_page_engine/910b_decode_hang_lanes_4fa19b4/`. Every 910B
lane passed at every swept position, including effective length 1280, and the
dense decode step cost a flat 2.9 ms from effective length 1024 through 3072.
These are controls only; they do not predict 310P behavior.
...
DENSE=09_persistent_page_engine/scripts/probes/probe_dense_decode_position_boundary.py
OP=09_persistent_page_engine/scripts/probes/probe_incre_flash_attention_boundary.py
RUNNER=09_persistent_page_engine/scripts/run_omnidocbench.py

"$PYTHON_BIN" "$DENSE" --help | grep -q -- '--pipeline-depth'
"$PYTHON_BIN" "$OP" --help | grep -q -- '--mask'
"$PYTHON_BIN" "$RUNNER" --help | grep -q -- '--decode-backend'

ROOT="$WORK_SERVER_REPO/tmp/09_persistent_page_engine/310p_phase31_hang_matrix_$COMMIT_SHORT"
test ! -e "$ROOT"
mkdir -p "$ROOT"
```

Constraints: do not edit tracked files, branch, commit, or push. Run each lane
in its own process. Terminate a hung process by its exact PID only; never
`pkill` or `killall`. Do not use AICore utilization as a causal diagnostic.

Every probe lane uses this position list, in this exact order. The suspect
position `1279` is deliberately **last** so that a hang there still leaves every
control position measured:

```text
POSITIONS=1023,1151,1277,1278,1280,1281,1282,1407,1535,1663,1791,2047,3071,1279
```

Effective length is `position + 1`, so `1279` is the reported failure and the
other entries include every 128-multiple boundary the same crop already passed
through without stalling.

### 31.2 Lane A: production page without the compiled decode graph

This is the highest-information lane. Take the exact Phase-29 Lane-A command
and change only the decode backend and the output directory:
...
Keep `--offset 11 --limit 1 --batch-size 16`, the diagnostic flags, min-pixels,
packing, PromptFA alignment, layout settings, cache length, and max-new-tokens
exactly as Phase 29 had them. `raw_eager` runs the same decode step with no
TorchAir graph, so no decode cache is used and none should be created.

Use the Phase-29 90-second no-progress evidence procedure, including
`ASCEND_PROCESS_LOG_PATH`, `last_scheduler_events.log`, `npu_smi_at_stall.txt`,
and the plog error grep.

| Lane A outcome | Conclusion |
| --- | --- |
| page completes | the compiled decode graph is the cause; the operator and the scheduler are not |
| still hangs at the target | the compiled graph is not the cause; continue to B and C |

### 31.3 Lane group B: whole decode step, no pipeline

```sh
run_lane () {
  NAME="$1"; shift
  mkdir -p "$ROOT/$NAME"
  timeout 1800 "$PYTHON_BIN" "$@" \
    --positions "$POSITIONS" \
    --output "$ROOT/$NAME/result.json" \
    >"$ROOT/$NAME/run.log" 2>&1 &
  PID=$!
  printf '%s\n' "$PID" >"$ROOT/$NAME/pid.txt"
  wait "$PID"
  printf '%s\n' "$?" >"$ROOT/$NAME/exit_code.txt"
}
```

| Lane | Command additions | Isolates |
| --- | --- | --- |
| `B1` | `--backend torchair --cache-init zeros --pipeline-depth 1` | position alone |
| `B2` | `--backend torchair --cache-init random --pipeline-depth 1` | reused-arena KV contents |
| `B3` | `--backend torchair --cache-init zeros --pipeline-depth 2` | the next step being in flight |
| `B4` | `--backend raw_eager --cache-init random --pipeline-depth 2` | everything except the graph |

Every B lane runs `$DENSE`. Reuse the Phase-29/30 decode cache root for the
torchair lanes with `--cache-dir`, and confirm from the emitted runtime
metadata that it reports `batch_size=16`, `cache_length=4096`,
`decode_attention=increfa`, and `decode_optimization=combined_apply`.

### 31.4 Lane group C: the attention operator alone

Every C lane runs `$OP`, which issues one `npu_incre_flash_attention` call at
the production decode shapes with no weights, projections, MLP, or lm_head.

| Lane | Command additions | Isolates |
| --- | --- | --- |
| `C1` | `--backend torchair --cache-init random --mask position` | the operator, compiled |
| `C2` | `--backend eager --cache-init random --mask position` | the operator, uncompiled |
| `C3` | `--backend torchair --cache-init random --mask none` | the mask itself |

`C3` passes `atten_mask=None`, so its output is position-independent by
construction. On 910B every C3 position returned the identical value; use that
as the correctness check that the lane really ran maskless.

### 31.5 Interpretation

Classify from the first lane that reproduces the stall:

| Observation | Conclusion |
| --- | --- |
| Lane A completes | TorchAir/GE decode graph; report it and rerun the Phase-27 throughput lanes with `--decode-backend raw_eager` to get 310P pages/s immediately |
| `C1` hangs at 1279, `C2` passes | compiled IncreFlashAttention; this is a minimal vendor reproduction |
| `C1` and `C2` both hang at 1279 | the IncreFlashAttention kernel itself; minimal vendor reproduction, no graph involved |
| `C3` passes while `C1` hangs | the position-built mask drives it, not the KV length |
| `B2` hangs while `B1` passes | reused-arena KV contents, not position; zeroing a leased slot is the production fix |
| `B3` hangs while `B1` passes | the two-in-flight enqueue pattern, not the step itself |
| every probe lane passes and Lane A hangs | the trigger needs real prefilled KV or real tokens; report and stop |
| a probe hangs at a position other than 1279 | 1280 is not unique; report the full set of failing effective lengths |
...
```text
310P PHASE 31 DECODE HANG MATRIX:
GRAPH | OPERATOR_COMPILED | OPERATOR_KERNEL | MASK | ARENA_CONTENTS |
IN_FLIGHT | REAL_STATE_DEPENDENT | UNRESOLVED

commit / host / exact NPU / software:
Lane A exact command / exit / page result:
Lane A last diagnostic_ events:
per-lane table: lane, exit code, last completed effective length, first
  effective length with no step_sync_end, per-position elapsed_s:
decode cache reused or recompiled per torchair lane:
first Python/CANN error, if any:
first matching plog error, if any:
mechanical classification from 31.5:
evidence paths:
```

Paste back `agent_report.md`, the complete `DENSE_DECODE_BOUNDARY` and
`INCRE_FA_BOUNDARY` progress records for every lane, and every Lane-A
`EXP09_SCHEDULER` line beginning with `diagnostic_`. Do not summarize the
progress records away.
